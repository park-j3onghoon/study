...
Codex 측은 자동 git 커밋·푸시·옵시디언 export를 수행하지 않는 점만 Claude 버전과 다름. 그 외 마이그레이션 절차는 동일.
...
Codex에서는 사용자가 세션 종료 처리를 명시 요청한 경우에만 갱신.
...
3. 사용자에게 추정값 확인 (Codex는 일반 질문 형식)
...
마이그레이션 후 다음 확인:
- `log/weakness.md` — 태그 prefix만 변경, 출현 횟수 등 보존
- `log/review-queue.md` — 빈 섹션 3개 (Due Today / Due This Week / Upcoming)
- 첫 학습 세션 실행 후 `meta/{개념}.json` 정상 생성 여부

## 롤백 (필요 시)

git revert 가능. lazy backfill로 추가된 frontmatter는 개별 파일 단위로 수정.
