...
FSRS(Free Spaced Repetition Scheduler) 기반으로 누적 학습 큐를 관리한다. concepts마다 `meta/{개념}.json`에 상태를 저장하고 `log/review-queue.md`로 집계.
...
3. 다음 섹션으로 분류:
   ...
   - **Upcoming**: `오늘+7일 < next_due ≤ 오늘+30일` (옵션, 길어지면 생략)
...
2. "Due Today" 섹션이 비어 있지 않으면 사용자에게 3지선다:
   - 복습 카드 {N}개 진행
   - 새 주제 학습
   - 둘 다 (복습 우선)
...
- 모든 주제 사전 지식이 충분할 때만 권장 (사용자가 옵트아웃 가능)
